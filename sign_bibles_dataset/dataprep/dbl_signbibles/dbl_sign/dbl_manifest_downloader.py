...
class HeadlessProgress:
    ...
    def update_file_progress(self, percent, message):
        print(f"[FILE] {percent:.1f}% - {message}")

    def update_overall_progress(self, percent, current, total):
        print(f"[OVERALL] {percent:.1f}% ({current}/{total})")
...
def download_manifest_files(
    ...
    progress_window = HeadlessProgress()
        ...
        try:
            ...
            try:
                ...
                for (
                    lang_code,
                    project_name,
                ), project_info in projects_to_process.items():
                    project_dir = base_dir / lang_code / project_name
                    if not s3_storage:
                ...
                progress_window.update_overall_progress(
                    overall_progress, current_mp4_count, file_limit or total_mp4_files
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--s3-folder",
        default="unprocessed",
        help="Folder in S3 bucket to upload files to",
    )
