...
def upload_to_huggingface(webdataset_path, dataset_name, token=None):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        manifest_path = os.path.join(project_root, "output", "manifest.json")
        if os.path.exists(manifest_path):
            print(f"Copying manifest.json to upload directory...")
...
def main():
    ...
    parser.add_argument("dataset_name", help="Name of the dataset on HuggingFace (e.g., 'username/dataset-name')")
    parser.add_argument("--token", help="HuggingFace API token (or set HUGGINGFACE_TOKEN environment variable)")
