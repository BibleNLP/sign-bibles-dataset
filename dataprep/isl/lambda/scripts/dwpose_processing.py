...
logging.basicConfig(filename='/isl-dataset-prep/logs/app.log', level=logging.INFO,
...
def generate_pose_files_v2(id):
    try:
      ...
      cap = cv2.VideoCapture(f"/isl-dataset-prep/isl_dict_videos/{id}.mp4")
      # width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
      # height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
      # fps = int(cap.get(cv2.CAP_PROP_FPS))
      cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
      ...
      while True:
          ...
          # video_writer2.write(skeleton)
      # video_writer2.release()
      np.savez_compressed(f"/isl-dataset-prep/outputs/{id}.pose-dwpose.npz",
